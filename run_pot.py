"""Method 5: PoT (Program of Thoughts) evaluation.

Generates Python programs expressing reasoning steps, then executes them.
Based on Chen et al. (2022) "Program of Thoughts Prompting".

Usage:
    CUDA_VISIBLE_DEVICES=0 python run_pot.py --model 4B --dataset gsm8k
"""
...
from prompts import POT_SYSTEM, POT_USER, build_messages
...
def evaluate(model, data, batch_size=32):
    ...
    all_prompts = []
    for ex in data:
        messages = build_messages(POT_SYSTEM, POT_USER, ex["problem"])
...
def main():
    parser = argparse.ArgumentParser(description="PoT evaluation")
